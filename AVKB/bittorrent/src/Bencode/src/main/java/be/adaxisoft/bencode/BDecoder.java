 ...
 */
package bittorrent.src.Bencode.src.main.java.be.adaxisoft.bencode;
...
public class BDecoder {
	...
	public static BEncodedValue decode(InputStream in) throws IOException {
		BEncodedValue value = new BDecoder(in).decode();
		in.close();
		return value;
	}
	...
	public static BEncodedValue bdecode(ByteBuffer data) throws IOException {
		return BDecoder.decode(new ByteArrayInputStream(data.array()));
	}
	...
	public BEncodedValue decodeMap() throws IOException {
		...
		Map<String, BEncodedValue> result = new HashMap<String, BEncodedValue>();
		c = this.getNextIndicator();
		while (c != 'e') {
